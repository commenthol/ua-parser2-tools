...
Models.prototype = {
	...
	_ref: function (brand, model) {
		...
		for (var i=5; i>0; i--) {
			...
			//~ if (model === 'A71') { // debugging
				//~ console.log(brand, model, tmpB, tmpM, device)
			//~ }
			...
			if (device.brand &&
				...
				if (deviceModels[device.brand] &&
					...
					if (!deviceModels[device.brand][device.model]) {
						deviceModels[device.brand][device.model] = device;
						return;
					}
				}
			}
		...
		console.log('Error: ', brand, model, device.brand, device.model, tmpB, tmpM);
